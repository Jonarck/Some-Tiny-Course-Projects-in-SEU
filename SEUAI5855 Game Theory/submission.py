...
def my_controller(observation, action_space, is_act_continuous=False):
    agent_action = []
    b = observation['state_map']
    # print(map)
    player = observation['chess_player_idx']
    if player==1:
        player=MAP_ENTRY_TYPE.MAP_PLAYER_ONE
    else:
        player=MAP_ENTRY_TYPE.MAP_PLAYER_TWO
    CHESS_LEN=15
    #map = Map(CHESS_LEN, CHESS_LEN)
    board=[[b[i][j][0] for j in range(15)] for i in range(15)]
    AI = ChessAI(CHESS_LEN)
    #AI_first = AI_RUN_FIRST
        ...
        for j in range(15):
            if board[i][j]!=0:
                ...
                if board[i][j]==1:
                    ...
                    AI.zobrist.generate(MAP_ENTRY_TYPE.MAP_PLAYER_ONE - 1, i, j)
                else:
                    AI.zobrist.generate(MAP_ENTRY_TYPE.MAP_PLAYER_TWO - 1, i, j)
                # if player == MAP_ENTRY_TYPE.MAP_PLAYER_ONE:
                #     player = MAP_ENTRY_TYPE.MAP_PLAYER_TWO
                # else:
                #     player = MAP_ENTRY_TYPE.MAP_PLAYER_ONE

    x, y = AI.findBestChess(board, player)
