...
public class Example15SimpleSPARQLQuery {
	...
	public static void main(String[] args)
		...
		try (RepositoryConnection conn = db.getConnection()) {
			...
			queryString += "SELECT ?creator \n";
			queryString += "WHERE { \n";
			queryString += "    {?creator ex:creatorOf ex:sunflowers.} \n";
			queryString += "    UNION{?creator ex:creatorOf ex:guernica.}";
			queryString += "}";
			...
			try (TupleQueryResult result = query.evaluate()) {
				...
				for (BindingSet solution : result) {
					...
					System.out.println("?creator = " + solution.getValue("creator"));
				}
			}
		} finally {
